...
```{r routines}
  ...
  build_expmodel <- function(data, 
    ...
    dayseq <- data$Days
    dayseq <- c(dayseq,(dayseq[length(dayseq)]+1):(dayseq[length(dayseq)]+9))
    dateseq <- data$Date
    dateseq <- as_date(c(dateseq,(dateseq[length(dateseq)]+1): 
                                 (dateseq[length(dateseq)]+9)))
    x <- data$Days
    y <- data[,indep][[1]]
    if (in_weights) {
    ...
    SD_lower <- Cases-Cases*std_dev
     ...
     SE.x   <- std_dev / sqrt(length(Cases))
     conf <-   Cases + Cases*qt(0.975, length(Cases) - 1) * SE.x
     
     preds <- tibble( Days=dayseq, Date=dateseq,!!indep:=Cases,
                        upper_conf=SD_upper, lower_conf=SD_lower) 
...
```{r crowd size}
  ...
  add_crowdsize <- function(p, data, PopLabel, 
                            in_mult, in_mult_pos, in_weights) {
    print(":::::::  add_crowdsize")
      begin <- data$Date[1] # date of first reported case
      LastDate <<- data[nrow(data),]$Date
      lastday <- as.integer(LastDate - begin) + 1  # last day of real data
...
CrowdText <- "Sizes of groups to avoid to keep\nchance of meeting a contagious\nperson below 1%"
      ...
      p <- p + CrowdLayer1 + 
               ...
               annotate("label", label=CrowdText, 
                        x=begin, y=Cases[length(Cases)]*0.9, 
...
prep_An_data('Region', 'Texas')
prep_An_data('County', 'Harris')
...
```{r build death plot}
  ...
  build_deaths_plot <- function(An_subdata,
                                in_weights=FALSE,
                                in_Deaths_logscale,
                                in_Deaths_zoom,
                                in_An_CFR,
...
#           Test Section
######################################################################
input$Deaths_logscale <- TRUE
input$weights <- FALSE
input$An_CFR <- 0.02
input$An_DeathLag <- 10
input$Deaths_back_est <- TRUE
input$Deaths_zoom <- TRUE
  #build_deaths_plot <- function(in_weights=FALSE){
      p <- build_deaths_plot(An_subdata,
                            input$weights,
...
data <-  prep_data(DF, 'Region', 'Houston-Galv')
data <-  prep_data(DF, 'County', 'Harris')

Asym <- max(data$Cases)*5
xmid <- max(data$Days)*2
scal <- 1/0.24
      
 ## fit using a selfStart model
 sigmoid <- nls(Cases ~ SSlogis(Days, Asym, xmid, scal), 
                 data=data)
 coeffs <- coef(sigmoid)
 
 #data <- data %>% 
 #  mutate(model=coeffs["Asym"]/(1 + exp((coeffs["xmid"] -
 #                                      Days)/coeffs["scal"])))
 projection <- 10
  dayseq <- data$Days
  ...
  #f.boot1 <- car::Boot(sigmoid)  # Be patient! Be aware of some non-convergence
  #confint(f.boot1)
  ...
  preds <- data.frame(dateseq, dayseq,
                     predict(sigmoid, data.frame(Days=dayseq)),
                     confint(f.boot))
  names(preds) <- c("Date", "Days","Cases","LCI","UCI")


 ggplot(preds,
        aes(x=Date, y=Cases)) +
   geom_ribbon(data=preds,aes(x=Date,ymin=LCI,ymax=UCI),fill="pink")+
   geom_point() +
...
foo <- readLines(paste0(path, file))
j <-  0
...
for (i in 1:length(foo)) {
 ...
 #####if (i > 150) {break}
  ...
  print(grepl("Submit an Entry", foo[i]))
...
## Let's play with slope and dailies

Calculate slope of cumulative curve via derivative, and via linear fit

Also calculate daily change, and smoothing functions for that quantity.
...
df2 <- df %>% 
  ...
  mutate(daily_cases=Cases-lag(Cases, 1, 0)) %>% 
  mutate(daily_deaths=Deaths-lag(Deaths, 1, 0))  
...
#df2 %>% mutate(smth = slide_dbl(daily_cases, 
#                                hanning, 
#                                .before = 2,
#                                .after = 2,
#                                .complete = TRUE))  %>% 
...
#       y="Positive Tests")
  #geom_line(aes(y=smth))
variable_names <- list(
  "daily_cases" = "New Cases in Texas" ,
  "daily_deaths" = "New Deaths in Texas"
)
...
df2 %>% 
  ...
  facet_wrap(~ type, ncol=1, scales="free", labeller=variable_labeller) +
...
df2 %>%
  mutate(
    ...
    slope = slide(
      ...
      #.f = ~coef(lm(log_cases ~ Days, .x))[["Days"]], 
      .f = ~lm(log_cases ~ Days, .x), 
      .before = 3, 
      .after = 3,
      .complete = TRUE
    ),
    tidied=map(slope, tidy)
  ) %>% 
  #unnest(slope) #%>% 
    unnest(tidied) %>% 
    filter(term=="Days") %>% 
  select(-Estimate, -log_cases, -slope, -term) %>% 
...
foo %>%  
  ...
  labs(title="Slope of Daily Cumulative Case Count",
       subtitle="Fit over 7 days")
