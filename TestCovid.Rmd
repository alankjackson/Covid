...
## Oh shit Major refactor of state website

```{r refactor}
tag <-  '/html/body/div/div/div/div/div/div/margin-container/full-container/div[11]/margin-container/full-container/div/div/nav'

url <- "https://txdshs.maps.arcgis.com/apps/opsdashboard/index.html#/ed483ecd702b4298ab01e8b9cafc8b83"
#   Bad idea, disabling certificate check
###page <- RCurl::getURL(url, ssl.verifyhost = 0L, ssl.verifypeer = 0L)
...
total_tests <- 
  read_html(parsed_pagesource) %>%
    #html_nodes(xpath='//*[@id="ember133"]') %>%
    html_nodes(xpath='//*[@id="ember16"]') %>%
    html_text() %>%  
    str_replace_all("\n"," ") %>% 
    str_remove_all(",") %>% 
    str_split("  +") 
total_tests <- as.numeric(total_tests[[1]][2])
public_tests <- 
  read_html(parsed_pagesource) %>%
    #html_nodes(xpath='//*[@id="ember129"]') %>%
    html_nodes(xpath='//*[@id="ember23"]') %>%
    html_text() %>%  
    str_replace_all("\n"," ") %>% 
    str_remove_all(",") %>% 
    str_split("  +")
public_tests <- as.numeric(public_tests[[1]][2])
private_tests <- 
  read_html(parsed_pagesource) %>%
    #html_nodes(xpath='//*[@id="ember131"]') %>%
    html_nodes(xpath='//*[@id="ember30"]') %>%
    html_text() %>% 
    str_replace_all("\n"," ") %>% 
    str_remove_all(",") %>% 
    str_split("  +")
private_tests <- as.numeric(private_tests[[1]][2])

testing_status <- tribble(
  ~Total,               ~Public,            ~Private,
  total_tests[[1]][2], public_tests[[1]][2], private_tests[[1]][2]
) %>%  
...
##  Build beginning of death table from news reports
...
# Read in the old data
TestingData <- readRDS("/home/ajackson/Dropbox/Rprojects/Covid/Deaths.rds")
# append the new data
TestingData <- bind_rows(TestingData, testing_status)
# Save an accumulated file in case of a failure
saveRDS(TestingData,paste0("/home/ajackson/Dropbox/Rprojects/Covid/",lubridate::today(),"_Deaths.rds"))
# Save the real file for later use
saveRDS(TestingData,"/home/ajackson/Dropbox/Rprojects/Covid/Deaths.rds")
# Also save to mirror site
saveRDS(TestingData,"/home/ajackson/Dropbox/mirrors/ajackson/Covid/Deaths.rds")
...
my_formula <- as.formula("log10(y) ~ x")
weights <- replicate(nrow(foo), 1)
x <- foo$Days
y <- foo$Deaths
...
## look at daily differences.

Is there an inflection point? Is the daily increase slowing?

Also compare different metro areas and counties.
...
writeBin(myfile, paste0(path, lubridate::today(),".xlsx"))
